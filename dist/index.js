...
async function getPg() {
  ...
  if (!url) return null;
  const client = new Client2({ connectionString: url, ssl: false });
  await client.connect();
  return client;
}
var PLAN_LIMITS = {
...
async function pingFirecrawl(key) {
  const start = Date.now();
  try {
...
async function pingScrapingBee(key) {
  const start = Date.now();
  try {
...
async function pingScrapfly(key) {
  const start = Date.now();
  try {
...
async function pingScreenshotOne(accessKey, secretKey) {
  const start = Date.now();
  try {
...
async function pingComposio(key) {
  const start = Date.now();
  try {
...
async function pingHelicone(key) {
  return key.startsWith("sk-helicone-") && key.length > 20 ? { ok: true, latencyMs: 0, message: "Format valid (sk-helicone-...)", endpoint: "(format check)" } : { ok: false, latencyMs: 0, message: "Invalid format \u2014 should start with sk-helicone-", endpoint: "(format check)" };
}
async function pingE2B(key) {
  const start = Date.now();
  try {
...
async function pingPinecone(key) {
  const start = Date.now();
  try {
...
async function pingInngest(key) {
  return key.length > 20 ? { ok: true, latencyMs: 0, message: "Format valid", endpoint: "(format check)" } : { ok: false, latencyMs: 0, message: "Key too short", endpoint: "(format check)" };
}
async function pingSteel(key) {
  return key.startsWith("ste-") ? { ok: true, latencyMs: 0, message: "Format valid (ste-...)", endpoint: "(format check)" } : { ok: false, latencyMs: 0, message: "Invalid format \u2014 should start with ste-", endpoint: "(format check)" };
}
...
async function pingGoogleOAuth(clientId) {
  return clientId.endsWith(".apps.googleusercontent.com") || clientId.length > 20 ? { ok: true, latencyMs: 0, message: "Format valid (Google client ID)", endpoint: "(format check)" } : { ok: false, latencyMs: 0, message: "Invalid format \u2014 Google client IDs end in .apps.googleusercontent.com", endpoint: "(format check)" };
}
async function pingMetaApp(appId) {
  return /^\d{10,20}$/.test(appId) ? { ok: true, latencyMs: 0, message: "Format valid (Meta App ID \u2014 numeric)", endpoint: "(format check)" } : { ok: false, latencyMs: 0, message: "Invalid format \u2014 Meta App ID should be a long numeric string", endpoint: "(format check)" };
}
async function pingTikTok(clientKey) {
  return clientKey.length > 10 ? { ok: true, latencyMs: 0, message: "Format valid", endpoint: "(format check)" } : { ok: false, latencyMs: 0, message: "Key too short", endpoint: "(format check)" };
}
async function pingReddit(clientId) {
  return clientId.length > 10 ? { ok: true, latencyMs: 0, message: "Format valid", endpoint: "(format check)" } : { ok: false, latencyMs: 0, message: "Key too short", endpoint: "(format check)" };
}
var PINGABLE_PROVIDERS = {
  OPENAI_API_KEY: {
    id: "OPENAI_API_KEY",
    name: "OpenAI",
    description: "GPT-based content generation (GPT-4o, GPT-4, etc.)",
  ...
  ANTHROPIC_API_KEY: {
    ...
    name: "Anthropic",
    description: "Claude-based content generation",
  ...
  GOOGLE_CLIENT_ID: {
    id: "GOOGLE_CLIENT_ID",
    name: "Google OAuth",
    description: "Google Business / YouTube OAuth",
  ...
  META_APP_ID: {
    id: "META_APP_ID",
    name: "Meta App",
    description: "Instagram / Facebook OAuth",
  ...
  TIKTOK_CLIENT_KEY: {
    ...
    name: "TikTok",
    description: "TikTok for Business OAuth",
  ...
  COMPOSIO_API_KEY: {
    id: "COMPOSIO_API_KEY",
    name: "Composio",
    description: "Universal OAuth hub (1000+ toolkits)",
  ...
  A2E_API_KEY: {
    ...
    ping: async (key) => ({ ok: !!key, latencyMs: 0, message: key ? "Set" : "Not set", endpoint: "(no live ping)" })
  },
  RESEND_API_KEY: {
    id: "RESEND_API_KEY",
    name: "Resend",
  ...
  SCRAPFLY_API_KEY: {
    id: "SCRAPFLY_API_KEY",
    name: "Scrapfly",
    description: "Premium scraping service",
    docsUrl: "https://scrapfly.io/dashboard",
    ping: pingScrapfly
  },
  SCREENSHOTONE_ACCESS_KEY: {
    id: "SCREENSHOTONE_ACCESS_KEY",
    name: "ScreenshotOne",
    description: "Screenshot generation API",
    docsUrl: "https://screenshotone.com/dashboard/",
    ping: (key) => pingScreenshotOne(key)
  },
  STEEL_API_KEY: {
    id: "STEEL_API_KEY",
    name: "Steel",
  ...
  E2B_API_KEY: {
    id: "E2B_API_KEY",
    name: "E2B",
    description: "Cloud sandboxed code execution",
  ...
  NVIDIA_API_KEY: {
    id: "NVIDIA_API_KEY",
    name: "NVIDIA NIM",
    description: "Free LLM inference (Llama, Mistral, etc.)",
...
function getProviderEnvValue(providerId) {
  const value = process.env[providerId];
  return value || "";
}
async function pingProvider(providerId, overrideKey) {
...
// server/_core/index.ts
init_env();
var BUILD_TAG = "cf-2026-07-02-00-08";
